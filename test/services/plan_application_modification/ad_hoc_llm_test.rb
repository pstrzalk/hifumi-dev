...
class PlanApplicationModification::AdHocLLMTest < ActiveSupport::TestCase
  ...
  test "passes system prompt and user prompt with intent to the LLM" do
    with_llm_response(plan_fixture("valid_plan.json")) do |captured|
      ...
      assert captured[:system].start_with?(PlanApplicationModification::AdHocLLM::SYSTEM_PROMPT)
  ...
  test "invoke_llm builds the chat on the pinned provider" do
    ...
    PlanApplicationModification::AdHocLLM.invoke_llm(system: "s", user: "u", openrouter_api_key: "sk-test", model: "anthropic/claude-haiku-4.5")
  ...
  test "system prompt sent to the LLM carries the photo inventory after the hand-written prompt" do
    with_llm_response(plan_fixture("valid_plan.json")) do |captured|
      PlanApplicationModification::AdHocLLM.call(intent: "todo list", clarifications: {}, context: {}, openrouter_api_key: "sk-or-test", model: "anthropic/claude-haiku-4.5")

      system = captured[:system]
      assert system.start_with?(PlanApplicationModification::AdHocLLM::SYSTEM_PROMPT),
