...
class Roast::WorkflowEnvTest < ActiveSupport::TestCase
  ...
  test "claude_model uses HIFUMI_DEV_MODEL when set" do
    assert_equal "haiku",
                 Roast::WorkflowEnv.claude_model({ "HIFUMI_DEV_MODEL" => "haiku" })
  end
  ...
  test "docs_model uses HIFUMI_DEV_DOCS_MODEL when set" do
    assert_equal "sonnet",
                 Roast::WorkflowEnv.docs_model({ "HIFUMI_DEV_DOCS_MODEL" => "sonnet" })
  end
  ...
  test "fix_budget_usd uses override when set" do
    assert_equal "1.25",
                 Roast::WorkflowEnv.fix_budget_usd({ "HIFUMI_DEV_FIX_BUDGET_USD" => "1.25" })
  end
  ...
  test "fix_budget_usd raises ArgumentError on non-numeric value (fail-fast at load)" do
    ...
    assert_raises(ArgumentError) do
      Roast::WorkflowEnv.fix_budget_usd({ "HIFUMI_DEV_FIX_BUDGET_USD" => "hello" })
    end
    assert_raises(ArgumentError) do
      Roast::WorkflowEnv.fix_budget_usd({ "HIFUMI_DEV_FIX_BUDGET_USD" => "" })
    end
  end

  # --- workspace ---------------------------------------------------------

  test "workspace raises a clear error when HIFUMI_DEV_WORKSPACE is missing" do
    err = assert_raises(RuntimeError) { Roast::WorkflowEnv.workspace({}) }
    assert_match(/HIFUMI_DEV_WORKSPACE/, err.message)
  end

  test "workspace returns the env value when set" do
    assert_equal "/tmp/ws",
                 Roast::WorkflowEnv.workspace({ "HIFUMI_DEV_WORKSPACE" => "/tmp/ws" })
  end
end
